...
const Card = ({ title, link, description, i, img, _id, bookmarked }) => {
  return (
    <a href={link} target="_blank" rel="noreferrer" className="relative">
      <motion.div
        className="card relative border-b-2 bg-gradient-to-r from-[#545454] to-[#807f7f] dark:from-gray-800 dark:to-gray-700 border-b-[#000000] dark:border-gray-600 flex flex-col rounded-lg gap-4 p-4 shadow-3xl h-[16rem] w-full hover:shadow-4xl transition-colors duration-200"
        key={i}
      ...
      >
        ...
        <div className="img img-container self-center">
          <img className="rounded-sm w-40 h-24 object-cover" src={img} alt="img" />
        </div>
